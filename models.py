...
class NeuralCF_new_self(nn.Module):

    def __init__(self,number_users,number_hstate,number_recipes,number_ingredients):
    

    
        super(NeuralCF_new_self,self).__init__()
        self.user_embedding = nn.Embedding(number_users+1, number_hstate, padding_idx=0)
        self.recipes_embedding = nn.Embedding(number_recipes+1, number_hstate, padding_idx=0)
        self.ingredients_embedding=nn.Embedding(number_ingredients+1,number_hstate, padding_idx=0)
        self.pad_idx=0
...
#        self.conv1=nn.Conv1d()
        ...
        self.lin41=nn.Linear(number_hstate*2,100)
        self.lin42=nn.Linear(100,50)
        self.dropout_3=nn.Dropout(p=0.3)
    ...
    def forward(self,x):
        ...
        recipe_emb=self.recipes_embedding(x['recipe'])
        # recipe_emb=recipe_emb[:,None,:]
        feat_concat_2=torch.concat((recipe_emb,user_emb),dim=1)

        r=self.lin41(feat_concat_2)
        r=F.relu(r)
        r=self.lin42(r)
        r=F.relu(r)
        r=self.dropout_3(r)
        ...
        #score = (r1*r2) #.sum(1)
        #print(score.shape,"shape")
        return torch.squeeze(final),health


class NeuralCF_pre_trained(nn.Module):

    def __init__(self,number_users,number_hstate,number_recipes,number_ingredients):
    

    
        super(NeuralCF_pre_trained,self).__init__()
        self.user_embedding = nn.Embedding(number_users+1, number_hstate, padding_idx=0)
        #self.recipes_embedding = nn.Embedding(number_recipes+1, number_hstate, padding_idx=0)
        self.ingredients_embedding=nn.Embedding(number_ingredients+1,number_hstate, padding_idx=0)
        self.pad_idx=0
...
#        self.conv1=nn.Conv1d()
    ...
    def from_pretrained(self):
     ...
     embeddings=torch.FloatTensor(np.load('ing_emb.npy'))
     self.ingredients_embedding.weight = torch.nn.Parameter(embeddings)
     #self.ingredients_embedding.weight.requires_grad =False
...
class NeuralCF_conv(nn.Module):

    def __init__(self,number_users,number_hstate,number_recipes,number_ingredients):
    

    
        super(NeuralCF_conv,self).__init__()
        self.user_embedding = nn.Embedding(number_users+1, number_hstate, padding_idx=0)
        #self.recipes_embedding = nn.Embedding(number_recipes+1, number_hstate, padding_idx=0)
        self.ingredients_embedding=nn.Embedding(number_ingredients+1,number_hstate, padding_idx=0)

        self.conv=nn.Conv1d(43,12,3,stride=1,padding='same')
        self.conv_1=nn.Conv1d(12,1,3,stride=1,padding='same')
    ...
    def forward(self,x):
        ...
        ing_emb=F.relu(self.conv(ing_emb))
        recipe_emb=F.relu(self.conv_1(ing_emb))
        recipe_emb=torch.squeeze(recipe_emb)
...
class NeuralCF_h_tags(nn.Module):

    def __init__(self,number_users,number_hstate,number_recipes,number_ingredients,number_tags):
    

    
        super(NeuralCF_h_tags,self).__init__()
        self.user_embedding = nn.Embedding(number_users+1, number_hstate, padding_idx=0)
        self.recipes_embedding = nn.Embedding(number_recipes+1, number_hstate, padding_idx=0)
        self.tag_embedding= nn.Embedding(number_tags+1, number_hstate, padding_idx=0)
        self.ingredients_embedding=nn.Embedding(number_ingredients+1,number_hstate, padding_idx=0)
        self.pad_idx=0
...
#        self.conv1=nn.Conv1d()
        ...
        self.att_i=nn.MultiheadAttention(number_hstate,8,batch_first=True,dropout=0.1)
        self.att_u=nn.MultiheadAttention(number_hstate,8,batch_first=True,dropout=0.1)
        self.att_t=nn.MultiheadAttention(number_hstate,8,batch_first=True,dropout=0.1)
        
     
        self.lin1=nn.Linear(2*number_hstate,100)
        self.lin2=nn.Linear(100,50)
        self.lin3=nn.Linear(50,1)
        self.dropout_1=nn.Dropout(p=0.3)
    ...
    def forward(self,x):
        ...
        padding_mask=(x['ingredients']==self.pad_idx)
        padding_mask_tags=(x['tags']==self.pad_idx)
        ...
        tag_emb=self.tag_embedding(x['tags'])


        recipe,weights=self.att_u(user_emb_1,ing_emb_feat,ing_emb_feat,key_padding_mask=padding_mask)
        recipe=torch.squeeze(recipe)

        tags,weights=self.att_t(user_emb_1,tag_emb,tag_emb,key_padding_mask=padding_mask_tags)
        tags=torch.squeeze(tags)

        feat_concat=torch.concat((tags,recipe),dim=1)
